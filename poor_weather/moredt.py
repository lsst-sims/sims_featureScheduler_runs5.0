...
from rubin_scheduler.site_models import Almanac, ScheduledDowntimeData, UnscheduledDowntimeMoreY1Data
...
def ddf_surveys(
    ...
    euclid_detailers : `list` of `rubin_scheduler.scheduler.Detailer`
        ...
        Default None.
    expt : `dict`
        Exposure time for DDF visits. Keys of band name, values
...
def make_downtimes(mjd_start):
    """Quick kludge to throw in 20 days of downtime in winter every year
    """
    mjd_start_time = Time(mjd_start, format="mjd")
    sched_downtime_data = ScheduledDowntimeData(mjd_start_time)
    unsched_downtime_data = UnscheduledDowntimeMoreY1Data(mjd_start_time)
    ...
    # add some extra 20 days in july-ish
    new_dt = np.zeros(10, dtype=list(zip(["start", "end"], [float, float])))
...
def gen_scheduler(args):
    ...
    roman_surveys = [
        gen_roman_on_season(nexp=nexp, exptime=STANDARD_EXP_TIME),
        gen_roman_off_season(nexp=nexp, exptime=STANDARD_EXP_TIME),
    ]
